...
DATA_DIR = '/media/laurence/Elements/Data' # path to the dataset
N_FRAMES = 1
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        file_idx = idx // 3
        indent = idx % 3
        folders = self.input_paths[file_idx].split('/')
        ...
        if self.use_estimations:
            with open(self.base_name + '/elastic_estimate.pkl', 'rb') as file:
                data = pickle.load(file).astype(np.float32)
            self.x_estimations[0] = torch.tensor(data,dtype=torch.float32).unsqueeze(0).unsqueeze(1)
            ...
            with open(self.base_name + '/hertz_estimate.pkl', 'rb') as file:
                data = pickle.load(file).astype(np.float32)
            self.x_estimations[1] = torch.tensor(data,dtype=torch.float32).unsqueeze(0).unsqueeze(1)
            #self.x_estimations[1] = torch.from_numpy(pickle.load(file).astype(np.float32)).unsqueeze(1)
        print(f"Estimates: {self.x_estimations}")

        # Unpack label
        self.y_label[0] = self.normalized_modulus_labels[file_idx]
...
class ModulusModel():
    ...
    def _execute(self, x_frames, x_forces, x_widths, x_estimations, batch_size=None):
    ...
    def start_estimation(self, grasp_data=None, E_hat_simple=None, E_hat_hertz=None, plot_results=False, verbose=True):
        
        self._estimation = True
        print('Start the Youngs modulus estimation')
        self._E_thread = Thread(target=self.thread_estimate, kwargs={"grasp_data": grasp_data, "E_hat_simple": E_hat_simple, "E_hat_hertz": E_hat_hertz, "plot_results":plot_results})
        self._E_thread.daemon = True
    ...
    def thread_estimate(self,grasp_data=None,E_hat_simple=None,E_hat_hertz=None,plot_results=False):
        ...
        if plot_results:
            ...
            fig, ax = plt.subplots()
            x_data, y_data = [], []  # Data to be plotted
            line, = ax.plot([], [], 'b-', label="E_NN")  # Primary y-axis (left)

            # Set up the axis limits and labels
            ax.set_xlim(0, 100)  # You can adjust these limits based on your data
            ax.set_ylim(-1, 1)
            ax.set_xlabel("Time (s)")
            ax.set_ylabel("E_NN", color="b")
        ...
        while self._estimation:
            ...
            if plot_results:
                ...
                if x >= ax.get_xlim()[1]:  # Extend the x-axis limit
                    ax.set_xlim(0, x + 10)
                if min(y_data) < ax.get_ylim()[0] or max(y_data) > ax.get_ylim()[1]:  # Adjust y1-axis
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'use_markers': True,                   # Use tactile images from sensor with markers
        ...
        'run_name': 'test_run_1frame_markers',                 # Name of W&B run
