...
class GraspData():
    ...
    def get_latest_data(self,last_n_frames=None):
        with shared_lock:
            ...
            self.contact_force._post_process_measurements()
            forces = self.contact_force.forces(last_n_frames).flatten()
            widths = self.gripper_width.widths(last_n_frames).flatten()
            return diff_images, depth_images, forces, widths
