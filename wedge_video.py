...
class GelSightWedgeVideo():
    ...
    def raw_RGB_frames(self,last_n_frames=None):
        if last_n_frames:
            return np.stack(self._raw_rgb_frames[-last_n_frames:], axis=0)
        else:
    ...
    def diff_images(self,last_n_frames=None):
        #ref_img = self.warped_RGB_frames()[0]
        ref_img = self.crop_image(self.warp_image(self._first_frame))
        # ref_img = cv2.GaussianBlur(ref_img, (13, 13), 0)
        if last_n_frames:
            self._diff_images = []
            for img in self.warped_RGB_frames(last_n_frames):
    ...
    def depth_images(self,last_n_frames=None):
        if last_n_frames:
            self._depth_images = []
            for frame in self.diff_images(last_n_frames):
    ...
    def _decode_image_from_stream(self):
        ...
        if a != -1 and b != -1:
            ...
            self._first_frame = self._raw_rgb_frames[0]
            self._last_frames = self._raw_rgb_frames[-3:]
...
if __name__ == "__main__":
    ...
    wedge_video = GelSightWedgeVideo(IP="172.16.0.3", config_csv="./wedge_config/config_no_markers.csv")
    wedge_video.start_stream(plot=True, plot_diff=True, plot_depth=False)
    time.sleep(20)
