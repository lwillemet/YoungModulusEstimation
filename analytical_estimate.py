...
class EstimateModulus():
    def __init__(
        ...
        self._contact_area_simple = []     
        self._contact_area_hertz = []
        self._x_data = []            # Save fitting data for plotting
        self._y_data = []            # Save fitting data for plotting

        self._E_elastic_thread = Thread         # Estim1 thread
        self._E_Hertz_thread = Thread           # Estim2 thread
    ...
    def top_percentile_depths(self, depth_images=None, percentile=97):
        ...
        return np.array(top_percentile_depths)*0.01
    ...
    def total_conditional_contact_mask(self, depth, depth_threshold=1e-4, threshold_pct=0.1):
        if self.max_depths(depth).max() > 0.001: # = depth_threshold / threshold_pct
    ...
    def contact_mask(self, depth, contact_mask=None):
        ...
        return self.mean_threshold_contact_mask(depth) # Default masking method
    ...
    def start_estimation(self, grasp_data=grasp_data, plot_results=False, verbose=True):
        
        self._estimation = True
        print('Start the Youngs modulus estimation')

        self._E_elastic_thread = Thread(target=self.thread_estimate, kwargs={"grasp_data": grasp_data, "plot_results": plot_results})
        self._E_elastic_thread.daemon = True
        self._E_elastic_thread.start()
        time.sleep(.5)

    # Terminate estimation thread
    def stop_estimation(self, verbose=False):
        self._estimation = False
        self._E_elastic_thread.join()
        
        time.sleep(1)
        if verbose: print('Stopping estimation.')

        return
    
    def thread_estimate(self,grasp_data=grasp_data, plot_results = False):
        n_frames_estim = 3
        if plot_results:
            # Set up the plot
            ...
            ax2 = ax1.twinx()
            x_data, y1_data, y2_data = [], [], []  # Data to be plotted
            line1, = ax1.plot([], [], 'b-', label="E_simple")  # Primary y-axis (left)
            line2, = ax2.plot([], [], 'r-', label="E_hertz")  # Secondary y-axis (right)
            ...
            ax2.set_ylabel("Contact area (mm2)", color="r")
        ...
        while self._estimation:
            ...
            sampled_diff_images, sampled_depth_images,sampled_forces,sampled_widths = grasp_data.get_latest_data(n_frames_estim)
            
            self.fit_modulus_simple(sampled_depth_images,forces=sampled_forces, widths=sampled_widths)
            self.fit_modulus_hertz(sampled_depth_images,forces=sampled_forces, widths=sampled_widths)

            if plot_results:
                x_data.append(x)
                #y1_data.append(self.E_simple)
                #y2_data.append(self.E_hertz)
                y1_data.append(sampled_forces[2])
                y2_data.append(self._contact_area_hertz*1e6)
                ...
                if x >= ax1.get_xlim()[1]:  # Extend the x-axis limit
                    ax1.set_xlim(0, x + 10)
                if min(y1_data) < ax1.get_ylim()[0] or max(y1_data) > ax1.get_ylim()[1]:  # Adjust y1-axis
                    ax1.set_ylim(min(y1_data) - 0.5, max(y1_data) + 0.5)
                if min(y2_data) < ax2.get_ylim()[0] or max(y2_data) > ax2.get_ylim()[1]:  # Adjust y2-axis
                    ax2.set_ylim(min(y2_data) - 0.5, max(y2_data) + 0.5)
                ...
                time.sleep(0.1)  # Adjust the sleep time for faster or slower updates
    ...
    def fit_modulus_simple(self, tactile_frames, forces=None, widths=None, contact_mask=None, depth_method=None, use_mean=False, use_ellipse_mask=False, \
        ...
        if use_lower_resolution_depth:
            depth_images = self.lower_resolution_depth(depth_images,kernel_size=5)
        else:
            depth_images = tactile_frames
        ...
        for i in range(len(depth_images)):
            ...
            if dL >= 0 and contact_area_i >= self.contact_area_threshold:
                ...
                y_data.append(abs(forces[i]) / contact_area_i) # Stress
        ...
        self._d = np.array(d)
        self._F = np.array(F)
        self._contact_area_simple =  np.mean(contact_areas)

        print(f"contact area...{contact_area_i}")
        if len(d) == 0:
        ...
        # Fit to modulus
        E = self.linear_coeff_fit(x_data, y_data)
        print(f"Simple estimate of Young's Modulus: {E:.3e} Pa")
        self.E_simple = E

        return E, np.mean(contact_areas)
    ...
    def fit_modulus_hertz(self, tactile_frames, forces=None, widths=None, contact_mask=None, use_ellipse_mask=False, fit_mask_to_ellipse=False, use_lower_resolution_depth=False):
        assert not (use_ellipse_mask and fit_mask_to_ellipse)
        ...
        self._d = np.array(d)
        self._F = np.array(F)
        self._contact_area_hertz = np.mean(contact_areas)
        ...
        self.E_hertz = E
        print(f"Hertzian estimate of Young's Modulus... {E:.3e} Pa")
    ...
    def fit_modulus_hertz_MDR(self, tactile_frames, forces=None, widths=None, contact_mask=None, depth_method=None, use_ellipse_mask=False, fit_mask_to_ellipse=False, \
        ...
        self._F = np.array(F)
        self._R = np.array(R)
        self._x_data = np.matrix.flatten(np.array(x_data))
        self._y_data = np.array(y_data)
        self._contact_area_hertz =  np.mean(contact_areas)
        ...
        print(f"Hertzian (MDR) estimate of Young's Modulus... {E_star:.3e} Pa")
        self.E_hertz = E_star

        return E_star,np.mean(contact_areas)
    ...
    def fit_modulus_hertz_MDR_both_sides(self, contact_mask=None, depth_method=None, use_ellipse_mask=False, fit_mask_to_ellipse=False, \
        ...
        if grasp_data is None:
            print("Using the grasp data from init...")
        else:
            self.grasp_data = grasp_data
...
if __name__ == "__main__":
    ...
    grasp_data              = GraspData(wedge_video=wedge_video, wedge_video_markers=wedge_video_markers, \
                                        contact_force=contact_force, gripper_width=gripper_width, use_gripper_width=False)
    ...
    for i in range(len(data_files)):
        ...
        estimator = EstimateModulus(grasp_data=grasp_data, use_gripper_width=False)
